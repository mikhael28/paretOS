  ...
  initialFetch = async (username) => {
    ...
    if (this.state.initialLoadComplete === false) {
      ...
      const user = await fetchUser(username).then((res) => {
        ...
        console.log(`user fetched in ${diff} seconds`);
        return res;
      });
      if (user.length > 0) {
        ...
        try {
          ...
          console.log("getting primary data...");
          ...
          const results = await Promise.all(
            ...
            endTime = new Date(Date.now()).getTime();
            const diff = (endTime - startTime) / 1000;
            console.log(`primary page data fetched in ${diff} seconds`);
            return res;
          });
          ...
          setTimeout(() => this.setCloseLoading(), 50);
        } catch (e) {
      ...
      try {
        ...
        let afterState = { ...newState };
        afterState.loading = false;
        afterState.initialLoadComplete = true;
        console.log("awaiting secondary data...");
        startTime = new Date(Date.now()).getTime();
        const afterResults = await Promise.all([
          ...
          endTime = new Date(Date.now()).getTime();
          const diff = (endTime - startTime) / 1000;
          console.log(`secondary data fetched in ${diff} seconds`);
          return res;
        });
        afterResults
          .filter((r) => r !== false)
